...
export default function SetlistEditorApp() {
  ...
  const handleSaveToMyAccount = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        showToast(t('setlists.savedSuccess', 'Երգացանկը պահպանվեց ձեր հաշվում'));
        fetchSetlist();
      } else {
            ...
            {setlistData.is_saved ? (
              <>
                <div style={{
                  ...
                  <span>{t('setlists.savedInYourAccountShort', 'Պահպանված է')}</span>
                </div>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{t('setlists.removeShort', 'Հեռացնել')}</span>
                </button>
              </>
            ) : (
              <button
                ...
                style={{
                  ...
                  boxShadow: '0 2px 8px rgba(0, 212, 255, 0.35)'
                }}
              >
                <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.5">
