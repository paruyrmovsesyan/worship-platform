...
export default function SetlistEditorWeb() {
  ...
  const allCoUsers = React.useMemo(() => {
    const map = new Map();
            ...
            <div>
              ...
              <div style={{ fontSize: '12.5px', color: 'rgba(255, 255, 255, 0.7)' }}>
                {setlistData.owner_name
                  ? t('setlists.sharedByAuthor', `Հեղինակ՝ ${setlistData.owner_name}։ Կարող եք պահպանել այն ձեր երգացանկերում՝ ազատ խմբագրելու համար։`)
                  : t('setlists.sharedSetlistHint', 'Կարող եք պահպանել այն ձեր հաշվում՝ ազատ խմբագրելու համար։')}
              </div>
            </div>
          </div>
          <button
            ...
            style={{
              ...
              border: 'none',
              cursor: 'pointer',
              boxShadow: '0 4px 14px rgba(0, 150, 255, 0.35)',
              whiteSpace: 'nowrap'
            }}
          >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                        {isOwner ? (
                          <>
                            ...
                            <button
                              type="button"
                              onClick={() => handleRevokeUserAccess(sUser.id, sUser.grantee_user_id, sUser.grantee_name || sUser.email)}
                              style={{
                                cursor: 'pointer',
                              ...
                              title="Հեռացնել համատեղ օգտագործողին"
                            >
          ...
          <div className="sl-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px' }}>
            <div className="sl-modal-header">
              <h3 className="sl-modal-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span>🤝</span>
                <span>{t('setlists.coUsersModalTitle', 'Համատեղ օգտագործողներ')}</span>
              </h3>
                          ...
                          {isOwner ? (
                            <>
                              <button
                                ...
                                style={{
                                  cursor: 'pointer',
                                  border: coUser.can_edit ? '1px solid rgba(46,204,113,0.4)' : '1px solid rgba(255,255,255,0.2)',
                                  background: coUser.can_edit ? 'rgba(46,204,113,0.2)' : 'rgba(255,255,255,0.08)',
                                  color: coUser.can_edit ? '#2ecc71' : 'var(--text-muted)',
                                  borderRadius: '6px',
                                  padding: '4px 10px',
                                  fontSize: '12px',
                                  fontWeight: '600',
                            ...
                            <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: coUser.can_edit ? 'rgba(46,204,113,0.2)' : 'rgba(255,255,255,0.1)', color: coUser.can_edit ? '#2ecc71' : 'var(--text-muted)' }}>
                              {coUser.can_edit ? 'Խմբագրող' : 'Դիտող'}
                            </span>
                          )}
                        </div>
