...
$authBadgeLabel = $isProgramAuth ? 'Worship ծրագիր' : 'Worship կայք';
$authTitle = $isProgramAuth ? 'Մուտք գործիր քո ծրագրային հաշվով' : 'Մուտք գործիր քո կայքի հաշվով';
$authLead = $isProgramAuth
  ? 'Մեկ մուտքով կստանաս պահպանված երգերը, սեթլիստները, push ծանուցումները և նույն փորձը թե կայքում, թե ծրագրի ներսում։'
  : 'Կայքում մուտքից հետո կբացվեն քո պահպանված երգերը, սեթլիստները և անձնական կարգավորումները, իսկ ծրագիրը կմնա նույն հաշվին կապված։';
$authMiniBadge = $isProgramAuth ? 'Ծրագրային մուտք' : 'Կայքի մուտք';
$authMetaChip = $isProgramAuth ? 'Քո տվյալները կապվում են հաշվին' : 'Հաշիվը միավորում է կայքն ու ծրագիրը';
$authNote = $isProgramAuth
  ? 'Մուտք գործիր, որպեսզի ծրագիրը բացի քո անձնական պահպանված երգերը, սեթլիստները և կարգավորումները։'
  : 'Մուտք գործիր, որպեսզի կայքում բացվեն քո պահպանված երգերը, սեթլիստները և հաշվի անձնական բաժինները։';
...
<title>Մուտք — Wolarm Youth</title>
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;font-family:'Inter',sans-serif;}
  ...
  .login-container{background:rgba(255,255,255,0.95);padding:40px;border-radius:16px;width:100%;max-width:380px;box-shadow:0 10px 25px rgba(0,0,0,0.2);color:#0f1222;}
  .login-container h2{text-align:center;margin-bottom:14px;font-size:24px;}
  .login-container input{width:100%;padding:12px 16px;margin-bottom:12px;border-radius:10px;border:1px solid #ccc;font-size:16px;}
  .login-container input:focus{outline:none;border-color:#3367ff;box-shadow:0 0 0 3px rgba(51,103,255,0.2);}
  .login-container button{width:100%;padding:12px 16px;background:linear-gradient(135deg,#3367ff,#2247d6);color:#fff;font-weight:700;border:none;border-radius:12px;cursor:pointer;font-size:16px;transition:all .2s ease;}
  .login-container button:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(51,103,255,0.3);}
  .error-msg{color:#ff3b30;margin-bottom:12px;text-align:center;font-weight:600;}
  .login-container p{text-align:center;font-size:14px;margin-top:12px;color:#555;}
  @media(max-width:400px){.login-container{padding:30px;}}
  .back-home {
    display:inline-block;margin-bottom:16px;text-decoration:none;font-weight:600;
    color:#3367ff;background:rgba(51,103,255,0.1);padding:8px 14px;border-radius:10px;transition:all .2s ease;
  }
...
.row a{
  padding:8px 10px;
  border-radius:12px;
  background: rgba(34,71,214,0.06);
}
.row a:hover{
  background: rgba(34,71,214,0.10);
}
  ...
  .links a{color:#2247d6;text-decoration:none;font-weight:600;}
...
/* reuse chip style + "ghost" variation */
.link-chip.ghost{
...
<body class="<?= htmlspecialchars($authBodyClass, ENT_QUOTES) ?>">
  <div class="auth-shell">
    <section class="auth-aside">
      ...
      <div class="auth-points">
        <div class="auth-point"><strong>Պահպանված երգեր</strong><span>Քո ընտրած երգերը միշտ կապվում են հաշվին։</span></div>
        <div class="auth-point"><strong>Սեթլիստներ</strong><span>Ծառայության երգացանկը հասանելի է նույն հաշվից։</span></div>
        <div class="auth-point"><strong>Push</strong><span>Ծրագրի թարմացումները և հայտարարությունները գալիս են հենց ներսում։</span></div>
        <div class="auth-point"><strong>Օֆֆլայն</strong><span>Մուտքից հետո ծրագրային փորձը մնում է ավելի ամբողջական և հարմար։</span></div>
      </div>
    </section>
    ...
    <h2>Մուտք</h2>
    <p class="auth-note"><?= htmlspecialchars($authNote, ENT_QUOTES) ?></p>
    ...
    <form method="POST" action="/loginuser.php">
      ...
      <input name="login" placeholder="Մուտքանուն կամ Էլ. փոստ" required>
      <input name="password" type="password" placeholder="Գաղտնաբառ" required>
  ...
  <span class="chk-text">Հիշել ինձ</span>
        ...
        <a class="link-chip" href="/registeruser.php?next=<?= htmlspecialchars($next, ENT_QUOTES) ?><?= htmlspecialchars($sourceQuery, ENT_QUOTES) ?>">
  Չունե՞ս հաշիվ
</a>
      </div>
...
<div class="social-auth">
  <div class="social-auth-sep">կամ շարունակիր</div>
    ...
    <?php foreach($socialProviders as $provider): ?>
      <?php
        ...
        $socialUrl = function_exists('wp_social_auth_start_url')
          ...
          : '#';
        $socialIcon = 'G';
        $socialNote = $providerEnabled ? 'Պատրաստ է մուտքի համար' : 'Միացրու ադմինից, որպեսզի աշխատի';
      ?>
      <a
...
<div class="links">
  ...
  Անուն կամ գաղտնաբառ մոռացե՞լ եք
</a>
</div>
  </div>
  </div>
